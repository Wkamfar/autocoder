...
- **`score_json.pipeline`** — structured as:
  ...
  - `raw`: key/value map of **every non-core column** except `segment` (strings), including firmographics (`employee_band`, `revenue_band`, `regions`, …), evidence (`evidence_url`, `hq_verified`, …), intro path, and risk tags (`primary_risk_vector`).
